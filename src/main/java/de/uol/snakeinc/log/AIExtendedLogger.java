 ...
 *
 * auto generated by the apache log4j extension tool. javadocs edited according to our checkstyle, extended so that ai
 * actions are saved in separate files for every game
 */
public final class AIExtendedLogger extends ExtendedLoggerWrapper {
    ...
    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy_MM_dd__HH_mm_ss");

    private static final String FQCN = AIExtendedLogger.class.getName();
    private static final Level AI = Level.forName("AI", 1);

    /**
     * creates a log file context for a game. There needs to be a delay between leaving two contexts, or their messages
     * will get mixed
    ...
    public boolean enterGame() {
        if (marker == null) {
            ...
            marker = MarkerManager.getMarker(DATE_TIME_FORMATTER.format(now));
            ai("enter game");
            return true;
        } else {
            return false;
    ...
    /**
     * exits a the games logfile context.
     */
